...
class y_quantize(nn.Module):
    ...
    def __init__(self, rounding, factor=None, arg=None):
    ...
    def forward(self, image):
        image = image.float() / (self.y_table @ self.factor).T.reshape(self.arg.batch_size, 8, 8).unsqueeze(1)
...
class c_quantize(nn.Module):
    ...
    def forward(self, image):
        image = image.float() / (self.c_table @ self.factor).T.reshape(self.arg.batch_size, 8, 8).unsqueeze(1)
        image = self.rounding(image)
        return image
...
class compress_jpeg(nn.Module):
    ...
    def __init__(self, rounding=torch.round, factor=None, arg=None):
        ...
        self.c_quantize = c_quantize(rounding=rounding, factor=factor, arg=arg)
        self.y_quantize = y_quantize(rounding=rounding, factor=factor, arg=arg)
