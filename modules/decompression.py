...
class y_dequantize(nn.Module):
    ...
    def __init__(self, factor=None, arg=None):
    ...
    def forward(self, image):
        return image * (self.y_table @ self.factor).T.reshape(self.arg.batch_size, 8, 8).unsqueeze(1)
...
class c_dequantize(nn.Module):
    ...
    def forward(self, image):
        return image * (self.c_table @ self.factor).T.reshape(self.arg.batch_size, 8, 8).unsqueeze(1)
...
class idct_8x8(nn.Module):
    ...
    def __init__(self):
        ...
        self.alpha = torch.from_numpy(np.outer(alpha, alpha)).float()
...
class decompress_jpeg(nn.Module):
    ...
    def __init__(self, height, width, rounding=torch.round, factor=None, arg=None):
        super(decompress_jpeg, self).__init__()
        self.c_dequantize = c_dequantize(factor=factor, arg=arg)
        self.y_dequantize = y_dequantize(factor=factor, arg=arg)
        self.idct = idct_8x8()
